...
# Cell
def plot_2d_fit(sta, param_d, fit_f=sum_of_2D_gaussian, imshow_args={"vmin":-1,"vmax":1, "cmap":"gray"}):
    """
    2D STA plotting helper function to quickly check results.

    params:
        - sta: Frame fitted containing the 2D STA
        - param_d: Parameter dictionary of the fit with the function in fit_f
    ...
    axes[1].imshow(sta, **imshow_args)

    return axes

def plot_chirpam_fit(cell_mean, param_d, fit_f=sinexp_sigm, QI=None,
                     start=420, stop=960, ax=None):
    """
    Helper funciton to visualize the fit of a cell response to a chirp_am stimulus.
    ...
    """
    if ax is None:
        fig, ax = plt.subplots(2,2, figsize=(50,2))
...
def plot_chirp_freq_epoch_fit(cell_mean, param_d_l, fit_f=sin_exponent, QI_l=[None]*5,
    ...
    """
    Helper funciton to visualize the fit of a cell response to a chirp_freq_epoch stimulus.

    params:
        - cell_mean: Cell's mean response to the stimulus
        - param_d_l: Parameters dictionary list of the fits with the function in fit_f
        - fit_f: Function used for the fit
    ...
    for len_fit, dur, params_d, QI in zip(len_fits, durations, param_d_l, QI_l):
        ...
        cursor += len_fit
    ax.xlim(0, len(cell_mean)/sampling_rate)
    bestfit = {k: round(v,2) for k, v in bestfit[0].items()}
    ax.title(str(bestfit))
...
# Cell
def plot_t_sta(sta, pval=None, ax=None):
    """
