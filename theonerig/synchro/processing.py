...
__all__ = ['get_thresholds', 'get_first_high', 'detect_frames', 'reverse_detection', 'error_check',
...
def detect_frames(data, low_threshold, high_threshold, increment, do_reverse=True):
    ...
    if do_reverse:
        ...
        frame_timepoints = new_timepoints + frame_timepoints
        frame_signals    = [0]*(len(new_timepoints)) + frame_signals
...
def frame_error_correction(signals, unpacked, algo="nw", **kwargs):
    """Correcting the display stimulus frame values. Shifts are first detected with òne
    of `shift_detection_conv` or `shift_detection_NW` and applied to the stimulus template. Then single frame
