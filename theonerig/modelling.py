...
__all__ = ['sigmoid', 'gaussian', 'gaussian_2D', 'sum_of_gaussian', 'sum_of_2D_gaussian', 'fit_sigmoid',
           'fit_spatial_sta', 'fit_temporal_sta', 'sin_exponent', 'sinexp_gauss', 'fit_chirp_am']
...
def fit_chirp_am(cell_mean, start=390, stop=960, freq=1.5):
    ...
    try:
        sinexp_gauss_part = partial(sinexp_gauss, freq=freq, exp=2)
        fit, cov = sp.optimize.curve_fit(sinexp_gauss_part, t, to_fit,
                                         bounds=[(1E-20, -np.inf,0,     0),
                                                (np.inf,np.inf,np.inf, 2*np.pi)])
        best_cov = cov[:3,:3]
        best_fit = (*fit, freq, 2)
        tmp_diff = np.sum(np.square(sinexp_gauss_part(t, *fit) - to_fit))
        phi = fit[3] #phi is from now on fixed
    except:
        best_fit = None
        best_cov = np.zeros((3,3)) + np.inf
        return best_fit, best_cov

    for exp in range(4,34,2): #Fitting the data with different sin exponents, to narrow the fit
        try:
            sinexp_gauss_part = partial(sinexp_gauss, phi=phi, freq=freq, exp=exp)
            fit, cov = sp.optimize.curve_fit(sinexp_gauss_part, t, to_fit, bounds=[(1E-20, -np.inf,0),
                                                                                   (np.inf,np.inf,np.inf)])
            mse = np.sum(np.square(sinexp_gauss_part(t, *fit) - to_fit))
            if mse < tmp_diff:
                best_cov = cov
